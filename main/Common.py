...
def normalize(X,objects,titles,type=[],mode=1):
    nre= len(X)
    nrf = len(titles)
    aList=[]
    if(len(type)==0):
        type=[0 for x in range(nrf)]
    for i in range(nre):
    ...
    for j in range(nrf):
        for i in range(nre):
            #empty bins are ignored in normalization - it might be beneficial in some cases (in most cases, it does not matter)
            if(mode==0):
            ...
            #even if bins have probability 0, they are considered - this option is better for State dataset
            elif(mode==1):
